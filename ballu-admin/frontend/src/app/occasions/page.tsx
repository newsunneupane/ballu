...
export default function OccasionsPage() {
  ...
  const [form, setForm] = useState({ name: { en: '', np: '' }, description: '', image: '' });
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState('');
  const imageInputRef = useRef<HTMLInputElement>(null);

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['occasions'] });

  const openNew = () => {
    setEditing(null);
    setForm({ name: { en: '', np: '' }, description: '', image: '' });
    setUploadError('');
    setShowForm(true);
  };
  ...
  const openEdit = (o: any) => {
    setEditing(o);
    setForm({ name: { en: o.name.en, np: o.name.np }, description: o.description || '', image: o.image || '' });
    setUploadError('');
    setShowForm(true);
  };
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await api.occasions.update(editing._id, payload);
      } else {
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const res = await api.upload.image(base64, 'ballu/occasions');
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-semibold text-[#1f1b16]">Occasions</h1>
      ...
      <div className="bg-[#ffffff] border border-[#e5ded2] rounded-lg overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {occasions.map((o: any) => (
              <tr key={o._id} className="border-b border-[#e5ded2]/50 last:border-0 hover:bg-black/5">
                <td className="px-5 py-3">
                  {o.image ? (
                    <img src={cloudinaryUrl(o.image, { width: 80 })} alt="" className="w-10 h-12 object-cover rounded border border-[#e5ded2]" />
                  ) : (
                    <span className="text-[#c9c2b4] text-xs">—</span>
                  )}
                </td>
                <td className="px-5 py-3 text-[#26221d]">{o.name.en}</td>
                <td className="px-5 py-3 text-[#26221d]">{o.name.np}</td>
                <td className="px-5 py-3 text-[#7d776c] text-xs">{o.description || '—'}</td>
                <td className="px-5 py-3 text-right">
                  <button onClick={() => openEdit(o)} className="text-[#7d776c] hover:text-[#b8860b] transition-colors mr-3"><Pencil size={14} /></button>
                  {o.name?.en?.toLowerCase() === 'others' ? (
                    <span className="text-[10px] tracking-[0.2em] uppercase text-[#b8860b]">Protected</span>
                  ) : (
                    <button onClick={() => remove(o._id)} className="text-[#7d776c] hover:text-red-600 transition-colors"><Trash2 size={14} /></button>
                  )}
                </td>
              </tr>
            ))}
            {occasions.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-[#6b655b]">No occasions yet</td></tr>
            )}
            {deleteError && (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-[#ffffff] border border-[#e5ded2] rounded-lg p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold text-[#1f1b16]">{editing ? 'Edit Occasion' : 'New Occasion'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name.en} disabled={editing && (editing.name?.en || '').toLowerCase() === 'others'} onChange={(e) => setForm({ ...form, name: { ...form.name, en: e.target.value } })} className="w-full bg-[#faf8f4] border border-[#e5ded2] rounded px-3 py-2 text-sm text-[#26221d] focus:outline-none focus:border-[#b8860b] disabled:opacity-60" />
              </div>
              <div>
                <label className="block text-[10px] tracking-[0.2em] uppercase text-[#6b655b] mb-1.5">Name (Nepali)</label>
                <input value={form.name.np} disabled={editing && (editing.name?.en || '').toLowerCase() === 'others'} onChange={(e) => setForm({ ...form, name: { ...form.name, np: e.target.value } })} className="w-full bg-[#faf8f4] border border-[#e5ded2] rounded px-3 py-2 text-sm text-[#26221d] focus:outline-none focus:border-[#b8860b] disabled:opacity-60" />
              </div>
              <div>
              ...
              <div>
                ...
                {form.image ? (
                  <div className="flex items-center gap-3">
                    <img src={cloudinaryUrl(form.image, { width: 160, aspect: '3:4' })} alt="Occasion" className="w-16 h-20 object-cover rounded border border-[#e5ded2]" />
                  ...
                  <>
                    <input
                      ...
                      id="occasion-image-input"
                    />
                    <label
                      htmlFor="occasion-image-input"
                ...
                <p className="text-[10px] text-[#6b655b] mt-1.5">Uploaded to Cloudinary and shown on the occasion card.</p>
              </div>
              {error && (
